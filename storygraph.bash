...
while [ -n "$page_titles" ]; do
  ...
  page_titles=$(echo "$page" | grep "img alt" | grep --invert-match StoryGraph | awk 'NR%2==1' | cut -d'"' -f2 | sed "s/&#39;/'/g" | sed '1d')
  page_imgs=$(echo "$page" | grep "img alt" | grep --invert-match StoryGraph | awk 'NR%2==1' | cut -d'"' -f6 | sed '1d')
